...
Usage - Single-GPU training:
    $ python train.py --data mydata.yaml --weights yolov5s.pt --img 640  # from pretrained (recommended)
    $ python train.py --data mydata.yaml --weights '' --cfg yolov5s.yaml --img 640  # from scratch

Usage - Multi-GPU DDP training:
    $ python -m torch.distributed.run --nproc_per_node 4 --master_port 1 train.py --data mydata.yaml --weights yolov5s.pt --img 640 --device 0,1,2,3
...
def train(hyp, opt, device, callbacks):
    ...
    Example:
        ...
        ```bash
        $ python -m torch.distributed.run --nproc_per_node 4 --master_port 1 train.py --data mydata.yaml --weights
...
def parse_opt(known=False):
    ...
    parser.add_argument("--data", type=str, default=ROOT / "data/yolo_insulator/data.yaml", help="dataset.yaml path")
...
def run(**kwargs):
    ...
    Args:
        ...
        data (str, optional): Path to dataset YAML configuration. Defaults to ROOT / 'data/mydata.yaml'.
    ...
    Examples:
        ```python
        import train
        train.run(data='mydata.yaml', imgsz=320, weights='yolov5m.pt')
        ```
